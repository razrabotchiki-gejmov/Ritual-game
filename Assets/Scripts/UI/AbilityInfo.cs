...
public class AbilityInfo : MonoBehaviour
{
    ...
    private void ActivateInfo(GameObject ability, GameObject info)
    {
        ...
        if (mousePos.x >= ability.transform.position.x - 50 && mousePos.x <= ability.transform.position.x - 50 + 100 &&
            mousePos.y >= ability.transform.position.y - 50 && mousePos.y <= ability.transform.position.y - 50 + 100)
        {
            info.SetActive(true);
        }
        else
